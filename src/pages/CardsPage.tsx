      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="flex justify-center items-start">
            <div
              className={`relative w-full max-w-md aspect-[1.586/1] rounded-xl shadow-2xl overflow-hidden transition-all duration-500 ${isFrozen ? 'grayscale opacity-80' : ''} bg-gradient-to-br from-blue-800 via-blue-900 to-gray-900 p-6 flex flex-col justify-between border border-gray-700`}>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div className="space-y-4">
              <button
                ...
                className={`w-full flex items-center justify-between p-4 border rounded-lg transition-colors ${isFrozen ? 'border-blue-200 bg-blue-50' : 'border-gray-200 hover:bg-gray-50'}`}>
                
                <div className="flex items-center">
                  <div
                    className={`w-10 h-10 rounded-full flex items-center justify-center mr-4 ${isFrozen ? 'bg-blue-100' : 'bg-blue-50'}`}>
                    
                    {isFrozen ?
                    <Unlock className="w-5 h-5 text-[#0060AF]" /> :

                    <Lock className="w-5 h-5 text-[#0060AF]" />
                    }
                  </div>
                ...
                <div
                  className={`w-12 h-6 rounded-full relative transition-colors duration-300 ${isFrozen ? 'bg-[#0060AF]' : 'bg-gray-300'}`}>
              ...
              <button
                ...
                className={`w-full flex items-center p-4 border rounded-lg transition-colors ${showDetails ? 'border-blue-200 bg-blue-50' : 'border-gray-200 hover:bg-gray-50'}`}>
                
                <div
                  className={`w-10 h-10 rounded-full flex items-center justify-center mr-4 ${showDetails ? 'bg-blue-100' : 'bg-blue-50'}`}>
                  
                  {showDetails ?
                  <EyeOff className="w-5 h-5 text-[#0060AF]" /> :

                  <Eye className="w-5 h-5 text-[#0060AF]" />
                  }
                </div>
                <div className="text-left">
                  <p className="font-medium text-gray-900">
              ...
              <button className="w-full flex items-center p-4 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">
                <div className="w-10 h-10 bg-blue-50 rounded-full flex items-center justify-center mr-4">
                  <RefreshCw className="w-5 h-5 text-[#0060AF]" />
                </div>
                <div className="text-left">
